...
if uploaded_file is not None:
    try:
        ...
        # [CORREÇÃO AUTOMÁTICA DA PLANILHA] Altera a legenda "A" para "Agd Dev Setor" em memória
...
# GRÁFICO 1: Status Temporal (Valores brutos em vez de porcentagem)
with row1_col1:
    st.subheader("1 Válidos, Vencidos, no Prazo")
    df_g1 = df["(Vencido, No Prazo, Prestes a Vencer)"].value_counts().reset_index()
    df_g1.columns = ["(Vencido, No Prazo, Prestes a Vencer)", "Quantidade"]
    
    if not df_g1.empty:
        fig1 = px.pie(
            df_g1, names="(Vencido, No Prazo, Prestes a Vencer)", values="Quantidade", hole=0.4,
            color_discrete_sequence=px.colors.qualitative.Pastel
        )
        # Força o gráfico a exibir o VALOR ABSOLUTO numérico (value) junto com o RÓTULO (label)
...
# GRÁFICO 2: Status por Documentos (Troca de "A" efetuada e Valores brutos ativos)
with row1_col2:
...
# ==============================================================================
# 5. GRÁFICOS INTERATIVOS COM MAPA DE CORES CUSTOMIZADO INDIVIDUALMENTE
# ==============================================================================

# [REDUÇÃO DE TÍTULO SOLICITADA] Gráfico 3
...
if status_disponiveis:
    ...
    if not df_g3.empty:
        ...
        # [CORES INDIVIDUAIS FIXAS] Vinculando cores hexadecimais a cada barra estritamente
        fig3 = px.bar(
            ...
            color_discrete_map={
                "APROVADO": "#2ca02c",
                "VERIFICADO AGUARDA DEVOLUÇÃO SETOR": "#d62728",
...
# ==============================================================================
# 6. ANÁLISE POR PROFISSIONAL (HISTÓRICO INTEGRADO VS FILTRO ATIVO)
# ==============================================================================
...
with row2_col1:
    ...
    # [CONTROLE DE EX-FUNCIONÁRIAS] Remove Sabrina e Sonalhya do filtro dinâmico
    ...
    if profissionais_ativos:
        ...
        # Se selecionar "Todos", a planilha inteira é processada (mantendo os dados históricos delas)
        ...
        if not df_g4.empty:
            ...
            # [CORES FIXAS POR STATUS] Mantendo o padrão visual idêntico em todas as visualizações de barras
            fig4 = px.bar(
                df_g4_counts, x="STATUS DO DOCUMENTO NORMATIVO",
                y="Quantidade", color="STATUS DO DOCUMENTO NORMATIVO",
            ...
            fig4.update_xaxes(title_text="") # Remove redundância visual do eixo X
...
# GRÁFICO 5: Documentos Aprovados por Tipo (Mantido fluxo original)
with row2_col2:
    ...
    if tipos_disponiveis:
        tipos_selecionados = st.multiselect(
            ...
            options=tipos_disponiveis, default=tipos_disponiveis
        )
