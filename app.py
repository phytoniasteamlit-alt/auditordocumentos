...
# ALTERADO: Nome da página modificado conforme solicitado
...
with col_titulo:
 # ALTERADO: Título modificado conforme solicitado para NORMA ZERO
...
if not df_base.empty:
 ...
 # ACRESCENTADO: Filtro para visualizar um documento aprovado específico individualmente por responsável
 documentos_validos = df_base["SIGLA"].dropna().unique()
 lista_documentos = sorted([str(d) for d in documentos_validos])
 ...
 # ALTERADO: Removido o subheader textual "Painel Executivo NAQH — Resultados Finais" deixando direto os gráficos
 ...
 with linha1_col1:
     ...
     # ALTERADO: Linha que tirava o "CANCELADO" foi removida para incluí-lo no gráfico perfeitamente
     st.bar_chart(dados_g1, color=c_g1_color, horizontal=True)
 
 with linha1_col2:
     # ALTERADO: Nome alterado estritamente para "Tempo de Análise" e retirada a "3º Verf."
     st.markdown("### Tempo de Análise")
     dados_g2 = pd.Series({
         "1º Verf.": media_v1,
         "2º Verf.": media_v2,
         "Total Estimado": (media_v1 + media_v2)
     })
     st.bar_chart(dados_g2, color=c_g2_color, horizontal=True)
 
 st.markdown("---")
 
 # ACRESCENTADO: Linha de gráficos intermediária para os prazos e volumes por tipo de documento
 linha2_col1, linha2_col2 = st.columns(2)
 
 with linha2_col1:
     # ACRESCENTADO: Gráfico focado nos prazos (Vencido, No Prazo, Prestes a Vencer)
     st.markdown("### Situação de Prazos")
     dados_prazo = df_filtrado["SIT_PRAZO"].dropna().value_counts()
     st.bar_chart(dados_prazo, color=c_g4_color, horizontal=True)

 with linha2_col2:
     # ACRESCENTADO: Gráfico que aponta a quantidade apenas de documentos APROVADOS por tipo (Manual, POP, Protocolo...)
