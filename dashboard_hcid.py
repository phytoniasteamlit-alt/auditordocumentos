...
# 1. Configuração da página profissional (Layout Expandido) st.set_page_config(     page_title="Painel de Indicadores Hospitalares",     layout="wide",     initial_sidebar_state="expanded" 
) 
 
# 2. Assinatura do Programador Fixada no Canto Inferior Esquerdo st.markdown( 
    """ 
    <style>     .footer {         position: fixed;         left: 15px;         bottom: 15px;         font-size: 14px;         font-weight: bold;         color: #888888;         z-index: 9999;         background-color: rgba(14, 17, 23, 0.9);         padding: 5px 10px;         border-radius: 5px; 
    } 
    </style> 
...
# 3. Menu Lateral de Controle with st.sidebar: 
    st.markdown("##   Painel de Controle")     st.write("Carregar Escala Hospitalar Completa (.xlsx):")     uploaded_file = st.file_uploader("Upload", type=["xlsx"], label_visibility="collapsed") 
     
    st.markdown("---")     st.markdown("###   Customização Visual") 
    cor_tema = st.selectbox("Tema de Cores Geral:", ["Padrão (Azul)", "Vibrante", 
"Warm"]) 
...
# 4. Cabeçalho Principal col_titulo, col_info = st.columns(2) with col_titulo: 
    st.markdown("#   Painel de Indicadores") 
 
with col_info:     st.markdown( 
        """ 
        <div style="text-align: right; padding-top: 15px; border-left: 2px solid #333; padding-left: 20px;"> 
            <span style="font-size: 20px; font-weight: bold;">  Hospital da 
Cidade</span><br> 
            <span style="font-size: 15px; color: #aaaaaa;">  Coord: Verônica 
Azevedo</span> 
        </div> 
        """, 
        unsafe_allow_html=True 
...
# 
======================================================================= == 
#   PROCESSADOR INTELIGENTE DE MATRIZ HOSPITALAR (MÁGICA DO PYTHON) 
# 
=======================================================================
== def processar_escala_complexa(df_raw): 
    # CORREÇÃO CRUCIAL: Nova sintaxe correta do ffill() para evitar o erro de iLocIndexer     df_raw[0] = df_raw[0].ffill()     df_raw[1] = df_raw[1].ffill()     df_raw[2] = df_raw[2].ffill() 
     
    linha_mes = df_raw.ffill()     linha_dia = df_raw.ffill()     linha_turno = df_raw 
     
    dados_estruturados = [] 
     
    for idx_linha in range(7, len(df_raw)):         linha_atual = df_raw.iloc[idx_linha] 
         
        setor_macro = str(linha_atual.iloc[0]).strip()         sub_setor = str(linha_atual.iloc[1]).strip()         categoria = str(linha_atual.iloc[2]).strip() 
        ...
        for idx_col in range(8, len(df_raw.columns)): 
            ...
            if pd.notna(valor_vaga) and (isinstance(valor_vaga, (int, float)) or str(valor_vaga).isdigit()): 
                qtd_vagas = int(valor_vaga)                 if qtd_vagas <= 0: 
                    continue 
                     
                mes = str(linha_mes.iloc[4, idx_col]).strip()                 dia = str(linha_dia.iloc[5, idx_col]).strip()                 turno = str(linha_turno.iloc[6, idx_col]).strip() if "manh" in turno.lower(): turno = "Manhã" 
                elif "tard" in turno.lower(): turno = "Tarde"                 elif "noit" in turno.lower(): turno = "Noite" 
...
# 5. Renderização e Execução do Upload if uploaded_file is not None: 
    try: 
        aba_hcid, aba_anexo = st.tabs(["  UNIDADE HCID", "  UNIDADE ANEXO"]) 
         
        # --- BLOCO VISUAL: HCID ---         with aba_hcid: 
...
mes_sel_hcid = st.multiselect("Selecione os Meses para Análise (HCID):", 
meses_hcid, default=meses_hcid)                 df_final_hcid = df_filtro_hcid[df_filtro_hcid["Mês"].isin(mes_sel_hcid)] 
        ...
        # --- BLOCO VISUAL: ANEXO ---         with aba_anexo: 
            try: 
df_raw_anexo = pd.read_excel(uploaded_file, sheet_name="ANEXO", 
header=None) 
                ...
                if not df_filtro_anexo.empty: 
                    ...
                    df_final_anexo = 
df_filtro_anexo[df_filtro_anexo["Mês"].isin(mes_sel_anexo)] 
                    ...
                    st.info("  Os gráficos do Anexo serão ativados automaticamente assim que houver registros de vagas maiores que zero na planilha.") 
            except Exception: 
                st.error("Aba 'ANEXO' não encontrada ou formato incompatível.") 
 
    except Exception as e: 
        st.error(f"  Erro crítico no processamento da matriz: {e}") 
