...
def extrair_numero(valor):
    if pd.isna(valor) or str(valor).strip() == "" or str(valor).strip().lower() == "nan":
        return 0
    # Isola exclusivamente os caracteres numéricos digitados na célula
...
# ==============================================================================
# 3. MOTOR DE PROCESSAMENTO DE CALENDÁRIO MATRICIAL (FIXO E RUST-PROOF)
# ==============================================================================
if uploaded_file is not None:
    ...
    # Carrega a tabela bruta sem cabeçalho para evitar perda de coordenadas
    df_raw = pd.read_excel(uploaded_file, sheet_name=aba_alvo, header=None)
    
    # MAPEAMENTO FIXO DOS CABEÇALHOS (Resolve colisões lógicas de linhas)
    # Linha 3 do Excel (índice 2), Linha 4 do Excel (índice 3), Linha 5 do Excel (índice 4)
    ...
    # Isola o corpo com os dados reais ignorando os títulos superiores (A partir da Linha 8 física / índice 7)
    ...
    # Varredura inteligente linha por linha da planilha convertendo a matriz em formato longo
    for _, row in df_corpo.iterrows():
        setor = str(row[0]).strip()
        sub_setor = str(row[1]).strip()
        categoria = str(row[2]).strip()
        
        # Filtro rígido para descartar ruídos textuais ou subtotais do Excel
        if "TOTAL" in setor.upper() or "TOTAL" in categoria.upper() or categoria == "" or setor == "GERAL":
            continue
            
        # Varre horizontalmente todas as colunas de calendário a partir da coluna D (índice 3)
        for col_idx in range(3, len(row)):
            ...
            if qtd_vagas > 0:
                ...
                # Filtro de validação temporal do calendário estrutural
                if any(m in mes for m in ["AGO", "SET", "OUT", "NOV", "DEZ"]) or "VAGAS" in mes or "PERIODO" in mes:
                    if "VAGAS" in mes or "PERIODO" in mes or mes == "": 
                        mes = "AGOSTO"
    ...
    # ==============================================================================
    # 4. EXIBIÇÃO INTERATIVA DAS CAIXAS DE TEXTO EXECUTIVAS (MÉTRICAS SOLICITADAS)
    # ==============================================================================
    if not df_master.empty:
        st.markdown("### 📋 Resumo Executivo de Capacidade (HCID)")
        
        # Consolidação matemática geral
        ...
        # Cria um seletor interativo para auditar os dias da semana
