...
def processar_escala_complexa(df_raw):
    # Preenche células mescladas verticais primárias (Setor e Categoria)
    df_raw.iloc[:, 0] = df_raw.iloc[:, 0].ffill()
    df_raw.iloc[:, 1] = df_raw.iloc[:, 1].ffill()
    df_raw.iloc[:, 2] = df_raw.iloc[:, 2].ffill()
    
    # Preenche linhas de cabeçalho da escala (Mês, Dia, Turno)
    ...
    for idx_linha in range(7, len(df_raw)):
        ...
        for idx_col in range(8, len(df_raw.columns)):
            ...
            if pd.notna(valor_vaga) and (isinstance(valor_vaga, (int, float)) or str(valor_vaga).isdigit()):
                ...
                mes = str(linha_mes.iloc[idx_col]).strip().upper()
    ...
    try:
        ...
        with aba_hcid:
            ...
            if not df_filtro_hcid.empty:
                ...
                # Gráfico 1 substituído por Card indicador de alta precisão
        ...
        with aba_anexo:
            try:
                ...
                if not df_filtro_anexo.empty:
                    ...
                    fig2_ax = px.histogram(df_final_anexo, x="Setor", title="2. Total de Setores Disponibilizados por Campo de Estágio no Anexo", color_discrete_sequence=[paleta_pasteis[0]])
                    st.plotly_chart(fig2_ax, use_container_width=True)
                    
                    df_g3_ax = df_final_anexo.groupby("Setor", as_index=False)["Vagas Ocupadas"].sum()
                    fig3_ax = px.bar(df_g3_ax, x="Setor", y="Vagas Ocupadas", title="3. Setores Disponibilizados para a Realização de Estágio no Anexo", color_discrete_sequence=[paleta_pasteis[1]], text_auto=True)
