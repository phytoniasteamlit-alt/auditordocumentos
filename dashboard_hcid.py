...
if uploaded_file is not None:
    ...
    if not df_hcid.empty:
        ...
        with col1_h:
            ...
            f1_h = px.bar(
                df_g1_h, 
                x="SETOR_RAW" if is_vert else "TOTAL_VAGAS", 
                y="TOTAL_VAGAS" if is_vert else "SETOR_RAW", 
                orientation="v" if is_vert else "h",
                text_auto=True, 
            ...
            f7_h = px.bar(
                df_melt_h, 
                x="SETOR_RAW" if is_vert else "VAGAS", 
                y="VAGAS" if is_vert else "SETOR_RAW", 
                color="TURNO", 
                barmode="group", 
                orientation="v" if is_vert else "h",
                text_auto=True, 
