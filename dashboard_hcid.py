...
# 1. Configuração da página profissional (Layout Expandido) - CORRIGIDO
st.set_page_config(
...
def processar_escala_complexa(df_raw):
    # Preenchimento lógico vertical que você definiu como correção crucial
    df_raw[0] = df_raw[0].ffill()
    df_raw[1] = df_raw[1].ffill()
    df_raw[2] = df_raw[2].ffill()
    
    # Preenchimento global horizontal para mesclados
    ...
    for idx_linha in range(7, len(df_raw)):
        ...
        for idx_col in range(8, len(df_raw.columns)):
            ...
            if pd.notna(valor_vaga) and (isinstance(valor_vaga, (int, float)) or str(valor_vaga).isdigit()):
                ...
                # Leitura exata nas linhas 4, 5 e 6 que fez seus gráficos aparecerem
    ...
    # Ordenação profissional cronológica para os gráficos de dias da semana
    ...
    with aba_hcid:
        try:
            ...
            if not df_filtro_hcid.empty:
                ...
                # G7 (Ordenado Cronologicamente)
                df_g7_hcid = df_final_hcid.sort_values("Dia da Semana")
                st.plotly_chart(px.bar(df_g7_hcid, x="Dia da Semana", y="Vagas Ocupadas", color="Turno", title="7. Total de Estagiários por Turno, por Dia, no HCID", barmode="group"), use_container_width=True)
            else:
                st.warning("Nenhum dado numérico de vagas encontrado na escala da aba HCID.")
        except Exception as e_hcid:
            st.error(f"Aba 'HCID' não encontrada ou formato incompatível. Detalhes: {e_hcid}")
    ...
    with aba_anexo:
        try:
            ...
            if not df_filtro_anexo.empty:
                ...
                # G7 (Ordenado Cronologicamente)
                df_g7_anexo = df_final_anexo.sort_values("Dia da Semana")
