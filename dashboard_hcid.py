...
def extrair_e_limpar_dados(uploaded_file, sheet_name):
    ...
    # Impede que erros de digitação ou textos invisíveis no cabeçalho sumam com os dados
...
# ==============================================================================
# 5. EXECUÇÃO DO FLUXO PRINCIPAL (LINHA RETA IMPOSSÍVEL DE GERAR INDENTAÇÃO)
# ==============================================================================
if uploaded_file is not None:
    excel_file = pd.ExcelFile(uploaded_file)
    abas_disponiveis = excel_file.sheet_names
    
    # Identifica as abas disponíveis
    aba_hcid_real = "HCID_BDD" if "HCID_BDD" in abas_disponiveis else abas_disponiveis[0]
    aba_anexo_real = "ANEXO" if "ANEXO" in abas_disponiveis else ("ANEXO2" if "ANEXO2" in abas_disponiveis else (abas_disponiveis[1] if len(abas_disponiveis) > 1 else None))
