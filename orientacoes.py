...
if "stats_texto" not in st.session_state:
    st.session_state.stats_texto = {
        "PAPEL": "NÃO", "MARGENS": "NÃO", 
        "MODELO DA FONTE E TAMANHO (CORPO DO TEXTO)": "NÃO", 
        "FONTE E TAMANHO (DENTRO DE TABELAS/HISTÓRICO)": "NÃO",
        "ESPAÇAMENTO ENTRE LINHAS": "NÃO", "ALINHAMENTO": "NÃO", "PARÁGRAFO": "NÃO", 
        "FIGURAS, TABELAS E GRÁFICOS": "NÃO", "PAGINAÇÃO": "NÃO", "MARCA D'AGUA": "NÃO", 
        "REFERÊNCIAS": "NÃO", "APÊNDICES/ ANEXOS": "OPCIONAL"
    }
...
if "corpo_texto_ok" not in st.session_state:
    st.session_state.corpo_texto_ok = True
if "tabelas_texto_ok" not in st.session_state:
    st.session_state.tabelas_texto_ok = True
...
if arquivo_word:
    ...
    # 1. Varredura Isolada do Corpo do Texto Geral (Exige Calibri 11 ou Arial 11)
    for p in doc.paragraphs:
        ...
        for r in p.runs:
            ...
            if nome_fonte and nome_fonte.upper() not in ["CALIBRI", "ARIAL"]:
                st.session_state.corpo_texto_ok = False
            if tamanho_fonte and int(tamanho_fonte) != 11:
                st.session_state.corpo_texto_ok = False

    # 2. Varredura Isolada das Tabelas / Registro Histórico (Exige Título 10pt Negrito e Dados 9pt)
    if len(doc.tables) > 0:
                ...
                for p_celula in celula.paragraphs:
                    ...
                    for r_celula in p_celula.runs:
                        ...
                        if is_registro_historico:
                            # Linha do Cabeçalho da Tabela -> Exige 10pt Negrito
                            if i_linha == 0:
                                if f_tam and int(f_tam) != 10:
                                    st.session_state.tabelas_texto_ok = False
                                    st.session_state.erros_formatacao.append(f"❌ **Tabela (Cabeçalho)**: O título '{text_clean[:15]}...' está com tamanho **{f_tam}pt** dentro do quadro de Histórico. O correto pela Norma Zero é **10pt (Negrito)**.")
                                if r_celula.bold is not True and len(text_clean) > 2:
                                    st.session_state.tabelas_texto_ok = False
                                    st.session_state.erros_formatacao.append(f"❌ **Tabela (Cabeçalho)**: O título '{text_clean[:15]}...' não está em Negrito. O correto é **10pt (Negrito)**.")
                            # Linhas de preenchimento de dados da tabela -> Exige 9pt
                            else:
                                if f_tam and int(f_tam) != 9 and int(f_tam) != 10:
                                    st.session_state.tabelas_texto_ok = False
                                    st.session_state.erros_formatacao.append(f"❌ **Tabela (Dados Internos)**: O preenchimento '{text_clean[:20]}...' está com tamanho **{f_tam}pt**. O correto pela Norma Zero para dados internos é **9pt (Justificado)**.")

    # 3. Varredura de Cabeçalhos textuais das seções
    ...
    # Atualiza as variáveis de texto de forma precisa e separada
    st.session_state.stats_texto["PAPEL"] = "SIM"
    st.session_state.stats_texto["MARGENS"] = "SIM"
    st.session_state.stats_texto["MODELO DA FONTE E TAMANHO (CORPO DO TEXTO)"] = "SIM" if st.session_state.corpo_texto_ok else "NÃO"
    st.session_state.stats_texto["FONTE E TAMANHO (DENTRO DE TABELAS/HISTÓRICO)"] = "SIM" if st.session_state.tabelas_texto_ok else "NÃO"
    ...
    # Realiza o cálculo de porcentagem baseado na lista corrigida
    lista_calculo = []
